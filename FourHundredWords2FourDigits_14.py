"This file is called FourHundredWords2FourDigits_14.py."
"It was forked from TwoHundredWords2FourDigits_7.py."
...
from array import *
my_filename = "FourHundredWords2FourDigits_14.py"
my_associated_file_extension = ".4H4D"
which_shuttle = "shuttle_3.txt"
...
"The output of shuttle output mode of this program can be read with ShuttleReader_3.py."

"""SUMMARY OF PROGRAM'S ATTRIBUTES
Name: FourHundredWords2FourDigits_14.py
...
def phi_ordnum(word_ST):
    ...
    if is_proper_keywordoid(word_ST):
        ...
        print("L167. ERROR in phi_ordnum: argument word_ST is not a proper wordoid.")
...
def is_string_a_decimal(st):
    ...
    if is_string(st):
        ...
        print("Error: argument st should be a string.\n")
        return False

def is_string_a_numeral(st):
    """Returns True if the string contains a number. (Numbers in scientific notation are
    not considered.)"""
    if is_string(st):
        if (len(st) > 0):
            if (st[0] == "-"):
...
def VAL(st):
    ...
    if is_string_a_numeral(st):
        ...
        print("L220. ERROR: string argument st should contain a number.")
...
def if_page_then_swallow(a_string):
    ...
    if (a_string[0:3] == "P. ") and (a_string[3] in digits_LS):
        ...
        if (len(a_string) > 0) and (a_string[0] == " "):
            ...
            print("L311. ERROR in if_page_then_swallow: No space after the page number.")
            a_string = ""
    return a_string
...
def if_paragraph_then_swallow(a_string):
    ...
    if (a_string[0:5] == "PAR. ") and (a_string[5] in digits_LS):
        ...
        if (len(a_string) > 0) and (a_string[0] == " "):
            ...
            print("L325. ERROR in if_paragraph_then_swallow: No space after the paragraph \
number.")
            a_string = ""
    return a_string
...
def is_UALPHAnumeric(a_string):
    ...
    return is_string(a_string) and a_string.isalnum() and (a_string == a_string.upper())

def input_UALPHAnumeric(message_ST):
    "Inputs an UALPHAnumeric string."
    value_ST = 999
    while not is_UALPHAnumeric(value_ST):
        value_ST = input(message_ST)
        if not is_UALPHAnumeric(value_ST):
            print("Error: The input must be numeric and/or uppercase alphabetic.")
...
def help_intro():
    ...
    while (page > 0):
        ...
        out_ST += "\nor X and <ENTER> to exit the help scroll."
        cont = input(out_ST)
        if (cont == "X"):
            page = 0
        elif (cont == "<"):
...
def help_page_6(opt):
    ...
    pr_pa("replace the apostrophe with a space (so that the “s” gets separated", opt)
    pr_pa("from the word and gets treated like a separate particle); that way", opt)
    pr_pa("the word is treated as a licit, seven-letter word instead of as", opt)
    pr_pa("an illicit eight-letter word.", opt)
...
def help_page_12(opt):
    ...
    pr_pa("“confirming” it), a flaw is noticed in a location string, then a way", opt)
    pr_pa("to deal with it can be to write a note about it on a piece of paper", opt)
...
def camouflage_trellis_alteration(digits_AR):
    "For alteration mode the submode is necessarily assignment (ss)."
    global output_submode
    output_submode = "ss"
    cont = True
    while cont:
        print("Just ahead: first camouflage trellis.\n")
        time.sleep(5)
        camouflage_trellis("\nCAMOUFLAGE TRELLIS FOR ALTERED PASSWORD", digits_AR)
        print("Just ahead: second camouflage trellis.\n")
        time.sleep(5)
        camouflage_trellis("\nCAMOUFLAGE TRELLIS FOR ALTERED PASSWORD", digits_AR)
        cont = whether_to_continue()    

def camouflage_trellis_modulator(title_ST, digits_AR):
    ...
    while not (output_submode in ["ss", "cc"]):
        output_submode = input(out_ST)
    if (output_submode == "ss"):
        cont = True
        while cont:
            print("Just ahead: first camouflage trellis.\n")
            time.sleep(5)
...
def camouflage_trellis_modulator_determined(title_ST, how_many_NT, digits_AR):
    "Asks about submode before calling camouflage trellis."
    out_ST = "Is this reconstruction session for newly assigning (ss) the given"
    out_ST += "\npassword (to some domain) or for accessing (cc) a password's"
    out_ST += "\n(already assigned) domain (ss/cc)? "
...
def password_shuttle(digits_AR):
    "Stores the password in digits_AR in a file referred to as 'password shuttle'."
    shuttle_position = 0
    while not (shuttle_position in ["1", "2"]):
        shuttle_position = input("What position: 1 or 2? ")
    if (shuttle_position == "1"):
        file_1 = open(which_shuttle, "w")
...
def password_shuttle_alteration(digits_AR):
    "In alteration mode the password can only be stored in the password shuttle's"
    "second position."
    file_1 = open(which_shuttle, "a")
    file_1.write(four_digit_mask_Mro_ST(digits_AR[0], digits_AR[1], digits_AR[2], \
                                        digits_AR[3]))
...
def swift_alter(old_digits_AR, is_for_exchange_NT):
    "Swift alteration mode. Global variables used: data, the_book, the_author, \
     ...
     is_for_exchange_NT must be one of -1, 0, 1. 1 means that it is for exchange; \
     0 means that it might be for exchange; -1 means that it is not for exchange."
    ...
    if not errorFlag:
        ...
        while not (answer_CH in ["y", "n"]):
            out_ST = "Would you like to try \"swift alter\" mode again "
            out_ST += "(\"y\", \"n\")? "
            answer_CH = input(out_ST)
            if not (answer_CH in ["y", "n"]):
                print("Error: please enter either \"y\" or \"n\".")
        if (answer_CH == "y"):
            return swift_alter(old_digits_AR, is_for_exchange_NT)       # recursive
        else:
            return 0
    ...
    if not errorFlag:
        ...
        if (ans_CH == "n"):
            ...
            while not filename_OK:
                ...
                if (len(f_ST) < 6) \
                   or not (f_ST[-5:] == my_associated_file_extension):
                    out_ST = "Error: The filename should have extension "
                    out_ST += f"{my_associated_file_extension}."
                    print(out_ST)
                elif (f_ST == old_f_ST):
            ...
            "output: swift alteration"
            output_mode = ""
            if (is_for_exchange_NT == 1):
                output_mode = "x"
            elif (is_for_exchange_NT == 0):
                out_ST = "\nCamouflage trellis (t), password shuttle (s), "
                out_ST += "or password exchange (x)? "                    
                output_mode = input_choice(out_ST, ["t", "s", "x"])
            elif (is_for_exchange_NT == -1):
                output_mode = input_choice(\
                    "\nCamouflage trellis (t) or password shuttle (s)? ", ["t", "s"])
            else:
                out_ST = "L1014. ERROR in swift_alter: is_for_exchange_NT should be one of"
                out_ST += "-1, 0, or 1."
                print(out_ST)
            if (output_mode == "t"):
                camouflage_trellis_alteration(digits)
            elif (output_mode == "s"):
                shuttle_pos = password_shuttle_alteration(digits)
            elif (output_mode == "x"):
                password_exchange(old_digits_AR, digits)
            ...
            if (antwoord_CH == "y"):
                ...
                with open(what_log, "a") as fil_chron:
                    ...
                    if (output_mode == "t"):
                        ...
                        if (output_submode == "ss"):
                            fil_chron.write("Output submode: assignment\n")
                        elif (output_submode == "cc"):
                            fil_chron.write("Output submode: access\n")                        
                    elif (output_mode == "s"):
                        fil_chron.write(f"Output mode: shuttle, #{shuttle_pos}\n")
                    elif (output_mode == "x"):
                        fil_chron.write("Output mode: exchange\n")
                print(f"A brief mention has been written in {what_log}.\n")
                "reminders: swift alteration"
                if (output_mode == "t"):
                    if (output_submode == "ss"):
                        print("If the password has just been assigned then")
                        print("record its metadata in the log for the password's")
                        print("domain, and make a backup or two of")
                        print("the file, including one in a flashdrive.\n")
                        print("Also play some mind-clearing game or puzzle.\n")
                    elif (output_submode == "cc"):
                        print("Play some mind-clearing game or puzzle.\n")
...
def what_index_words(datum_ST):
    "Extracts the index words from datum_ST and returns them."
    return if_paragraph_then_swallow(if_page_then_swallow(datum_ST))

def if_not_is_proper_keyword(word_ST):
    """Deals with argument word_ST not being a proper keyword. If it is a correction
    request, addition request, or replacement request, then deal with it; otherwise
    give an error message."""
    global data, prefix, reconstruct_changed
    if not is_proper_keyword(word_ST):
        if (i > 1) and (word_ST == ":CORRECTION:"):
            out_ST = f"Current previous (#{i - 1}) location string: "
            out_ST += f"{data[i - 1][0]}"
            print(out_ST)
                ...
                if not (new_prefix == prefix):
                    if not (page_num == "0"):
                        ...
                        data[i - 1] = (new_prefix + data[i - 1][0], \
                                       ...
                                       data[i - 1][3])
                        out_ST = f"New previous (#{i - 1}) location string: "
                        out_ST += f"{data[i - 1][0]}"
                        print(out_ST)
                        reconstruct_changed = True
                        prefix = new_prefix
                    else:
                        print("CORRECTION edit cancelled.\n")
                else:
                    print("Error: The same location prefix should")
                    print("not be repeated.")
                print("\nNow back to the present entry.")
                print(f"\nLocation #{i}: {data[i][0]}")
                if (what_prefix(data[i][0]) != ""):
                    prefix = what_prefix(data[i][0])
        ...
        elif (i > 1) and (word_ST == ":ADDITION:"):
            out_ST = f"Current previous (#{i - 1}) location string: "
            out_ST += f"{data[i - 1][0]}"
            print(out_ST)
            prependix = input_proper_addition("Enter the word(s) to prepend: ")                
            beginning_part = what_prefix(data[i - 1][0])
            ending_part = what_index_words(data[i - 1][0])
            new_datum = beginning_part + prependix + " " + ending_part
            ...
            print("Error: It is not possible to add index words to a non-existent")
            print("zeroth entry.")
        elif (i > 1) and (word_ST == ":REPLACEMENT:"):
            beginning_part_ST = what_prefix(data[i - 1][0])
            ending_part_ST = what_index_words(data[i - 1][0])
            word_AR = ending_part_ST.split(" ")
            index_NT = input_integer(\
                "What word to replace? (enter an index) (indices start at 0) ")
            print(f"Word to be replaced: {word_AR[index_NT]}")
            replacement_ST = input_UALPHAnumeric("Replacing word? ")
            if not (replacement_ST == word_AR[index_NT]):                
                word_AR[index_NT] = replacement_ST
                string_ST = synthesize_string(word_AR)
                new_datum_ST = beginning_part_ST + string_ST
                data[i - 1] = (new_datum_ST, data[i - 1][1], data[i - 1][2], data[i - 1][3])
                print(f"New previous (#{i - 1}) location string: {data[i - 1][0]}")
                reconstruct_changed = True
            print("\nNow back to the present entry.")
            print(f"\nLocation #{i}: {data[i][0]}")
        elif (i == 1) and (word_ST == ":REPLACEMENT:"):
            print("Error: It is not possible to replace a word in a non-existent")
            print("zeroth entry.")
        elif (i > 1) and (word_ST == ":INSERTION:"):
            beginning_part_ST = what_prefix(data[i - 1][0])
            ending_part_ST = what_index_words(data[i - 1][0])
            word_AR = ending_part_ST.split(" ")
            index_NT = input_integer(\
                "Where to insert? (enter an index) (indices start at 0) ")
            insertion_ST = input_UALPHAnumeric("Inserted word? ")
            word_AR = word_AR[:index_NT] + [insertion_ST] + word_AR[index_NT:]
            string_ST = synthesize_string(word_AR)
            new_datum_ST = beginning_part_ST + string_ST
            data[i - 1] = (new_datum_ST, data[i - 1][1], data[i - 1][2], data[i - 1][3])
            print(f"New previous (#{i - 1}) location string: {data[i - 1][0]}")
            ...
            print("Error: It is not possible to insert a word in a non-existent")
            print("zeroth entry.")
        elif (i > 1) and (word_ST == ":DELETION:"):
            beginning_part_ST = what_prefix(data[i - 1][0])
            ending_part_ST = what_index_words(data[i - 1][0])
            word_AR = ending_part_ST.split(" ")
            index_NT = input_integer(\
                "Where to delete? (enter an index) (indices start at 0) ")
            word_AR = word_AR[:index_NT] + word_AR[(index_NT + 1):]
            string_ST = synthesize_string(word_AR)
            new_datum_ST = beginning_part_ST + string_ST
            data[i - 1] = (new_datum_ST, data[i - 1][1], data[i - 1][2], data[i - 1][3])
            print(f"New previous (#{i - 1}) location string: {data[i - 1][0]}")
            ...
            print("Error: It is not possible to delete a word in a non-existent")
            print("zeroth entry.")
        else:                        
            print("Error: Please enter an eight-letter word in all caps.")

def is_proper_addition(words_ST):
    "Checks to see if words_ST has a correct format to serve as addend to location string."
    if not is_string(words_ST):
        return False
    if words_ST.count("  ") > 0:
        return False
...
def input_proper_addition(message_ST):
    """Asks for an input of one or more all-caps words (or numbers) (using space as \
    separator if necessary)."""
    value_ST = 999
    while not is_proper_addition(value_ST):
        value_ST = input(message_ST)
        if not is_proper_addition(value_ST):
            print("Error: The input should be a string made up of uppercase")
            print("letters and/or digits and (possibly) single spaces between them.")
            print("Also, there should be no space either at the beginning or at the")
            print("end. Please try again.\n")            
    return value_ST

def password_exchange(old_digits_AR, digits_AR):
    """For use with swift alter: Presents a single camouflage trellis for the
    older password, contained in old_digits_AR, followed by a double camouflage trellis
    for the replacement password, contained in digits_AR."""
    camouflage_trellis_modulator_determined("\nCAMOUFLAGE TRELLIS FOR THE OLD PASSWORD", \
                                            1, old_digits_AR)
    camouflage_trellis_modulator_determined("\nCAMOUFLAGE TRELLIS FOR THE NEW PASSWORD", \
                                            2, digits_AR)

def synthesize_string(words_AR):
    """Put together elements of words_AR (which should be strings) into a single string,
    separating adjacent elements with single spaces."""
    string_ST = ""
    for elem_ST in words_AR:
        string_ST += " " + elem_ST
...
def input_choice(message_ST, choices_LS):
    """Asks for an input from among a list of choices and loops until it gets a proper
    answer. The choices in list choices_LS must be strings."""
    choice_ST = 999
    while not (choice_ST in choices_LS):
        choice_ST = input(message_ST)
        if not (choice_ST in choices_LS):
            if (len(choices_LS) == 2):
                print("Error. Please enter either " + express_disjunction(choices_LS) + ".")
            else:
                print("Error. Please enter one of " + express_disjunction(choices_LS) + ".")
    return choice_ST

def express_disjunction(strings_LS):
    "Returns a string which lists out the elements of strings_LS."
    if (len(strings_LS) == 0):
        return "{}"
    elif (len(strings_LS) == 1):
        return strings_LS[0]
    elif (len(strings_LS) == 2):
        return strings_LS[0] + " or " + strings_LS[1]
    elif (len(strings_LS) > 2):
        output_ST = strings_LS[0]
        for element_ST in strings_LS[1:-1]:
            output_ST += ", " + element_ST
        output_ST += ", or " + strings_LS[-1]
        return output_ST
...
if is_toy_version and is_trial_version:
    print("L1143. ERROR: is_toy_version and is_trial_version should not")
...
"LOAD?"
mode = input_choice("Create new (c), reconstruct (r), or alter (a)? ", ["c", "r", "a"])
if (mode == "r") or (mode == "a"):
    filename_OK = False
    while not filename_OK:
        f_ST = input("Enter filename: ")
        if (len(f_ST) < 6) or (f_ST[-5:] != my_associated_file_extension):
            out_ST = "Error: The filename's extension should be "
            out_ST += f"{my_associated_file_extension}"
            print(out_ST)
        else:
            filename_OK = True    
        ...
        for line in f:
            ...
            if (line[0] == "(") and (line[-1] == ")"):
                ...
                if len(line_LS) == 4:
                    ...
                    if not line_LS[0] in ["Executor:", "Mode:", "Book:", "Author:", \
                                          "Publisher:", \
                                          "Location:", "Codeword hash:"]:
                        print("L1092. ERROR: Not an approved first element for an")
                        print("ordered pair.")
                    if (line_LS[0] == "Executor:") \
                       and (line_LS[1] != my_filename): 
                    ...
                    if line_LS[0] == "Mode:":
                        ...
                        if (the_mode == "toy") != is_toy_version:
                            ...
                            print("mode.")
                            quit()
                        elif (the_mode == "trial") != is_trial_version:
                            print("Error: the executor's mode does not match the file's")
                            print("mode.")
                            quit()
                    elif line_LS[0] == "Book:":
...
"MAIN"
errorFlag = False
mm = array('i', [100, 200, 300, 400])
nn = array('i', [10, 20, 30, 40])
keywords_LS = []
if (mode == "c"):
    data = [0]
old_vector_left_LS = [0, 0, 0, 0, 0, 0, 0, 0]
...
if is_toy_version:
    ...
    upper_bound = mm[3]
    if is_trial_version:
        print("This execution is in trial mode.")
if (mode == "c"):
...
for i in range(1, upper_bound + 1):
        ...
        while enter_loop_BL:
            ...
            if (loc == ":HELP:"):
                ...
                if (data[i - 1][0].count(".") == 0):
                    ...
                    if not (new_prefix == prefix):
                        if not (page_num == "0"):
                            ...
                            print("CORRECTION has been cancelled.\n")
                        word_ST = "ZZZZZZZZ"
                        OK = True
                        respuesta = "N"
                    else:
                    ...
                    out_ST = "L1209. ERROR: Wrong number of periods"
                    out_ST += f" ({data[i - 1][0].count('.')}) in the location string."
                    print(out_ST)
            elif (i == 1) and (loc == ":CORRECTION:"):
                ...
                if not (replacement_ST == word_AR[index_NT]):
                    ...
                    string_ST = synthesize_string(word_AR)
                    new_datum_ST = beginning_part_ST + string_ST
                    data[i - 1] = (new_datum_ST, data[i - 1][1], data[i - 1][2], \
                                   data[i - 1][3])
                    print(f"New previous (#{i - 1}) location string: {data[i - 1][0]}")
                word_ST = "ZZZZZZZZ"
                OK = True
                ...
                response = input_choice(\
                    "Continue anyway (\"CONTINUE\") or retry (\"RETRY\")? ", \
                    ["CONTINUE", "RETRY"])
                if (response == "RETRY"):
                    OK = True
                    respuesta = "N"
                    ...
                    if (len(what_prefix(loc)) > 0):
                        if (what_prefix(loc) == prefix):
                            ...
                            DEBUG(debug_1, f"L1264. prefix = {prefix}")
                            prefix_changed = True
                    else:
                        older_prefix = prefix
            else:
                ...
                if (len (what_prefix(loc)) > 0):
                    if (what_prefix(loc) == prefix):
                        ...
                        older_prefix = prefix
                        prefix = what_prefix(loc)
                        DEBUG(debug_1, f"L1279. prefix = {prefix}")
                        prefix_changed = True
                else:
                    older_prefix = prefix
            while not OK:
            ...
            if (word_ST == "XXXXXXXX"):
                ...
                if prefix_changed:
                    prefix = older_prefix
                    DEBUG(debug_1, f"L1300. prefix = {prefix}")
            elif (word_ST == "ZZZZZZZZ"):
                OK = True
                respuesta = "N"
                if prefix_changed:
                    prefix = older_prefix
                    DEBUG(debug_1, f"L1306. prefix = {prefix}")
                ...
                if prefix_changed:
                    prefix = older_prefix
                    DEBUG(debug_1, f"L1313. prefix = {prefix}")
            else:
                OK = False
                time.sleep(1)
            while not OK:
    ...
    for ii in [0, 1, 2, 3]:
        if (i == mm[ii]) or (is_toy_version and (i == nn[ii])):
            DEBUG(debug_2, f"L1466. i = {i}, ii = {ii}")
            vecsum_left[ii] = old_vector_left_LS
            vecsum_right[ii] = old_vector_right_LS
            vecsum_total[ii] = vecsum_left[ii] + vecsum_right[ii]
            DEBUG(debug_2, f"L1470. vecsum_left[{ii}] = {vecsum_left[ii]}")
            DEBUG(debug_2, f"L1471. vecsum_right[{ii}] = {vecsum_right[ii]}")
            DEBUG(debug_2, f"L1472. vecsum_total[{ii}] = {vecsum_total[ii]}")
    "Fingerprint (of a keyword)."
    lett = phi_lett(vector_LS)
    variance = phi_variance(vector_LS)
    ordnum = phi_ordnum(word_ST)
...
old_digits = [-1, -1, -1, -1]
for ii in [0, 1, 2, 3]:
    DEBUG(debug_2, f"L1484. vecsum_total[{ii}] = {vecsum_total[ii]}")
    digits[ii] = vector_hash_6(vecsum_total[ii]) % 10
    old_digits[ii] = digits[ii]
...
if (mode == "r"):
    if (codeword_hash == the_codeword_hash):
        ...
        print("L1512. ERROR: Output's hash is NOT OK.")
...
if (not errorFlag) and (mode == "r"):
    out_ST = "Camouflage trellis (t), password shuttle (s), or "
    out_ST += "wait to exchange with a subsequent swift alteration (w)? "        
    output_mode = input_choice(out_ST, ["t", "s", "w"])

if (not errorFlag) and (mode == "r") and (output_mode == "t"):
    camouflage_trellis_modulator("\nCAMOUFLAGE TRELLIS FOR RECONSTRUCTED PASSWORD", digits)
...
"output: creation"
if (mode == "c"):
    ans_CH = input_y_or_n("Save (y/n)? ")
    if ans_CH == "y":
        data.append(("Executor:", my_filename))
        if is_toy_version:
        ...
        if (antwoord_CH == "y"):
            ...
            with open(what_log, "a") as fil_chron:
                ...
                if errorFlag:
                    fil_chron.write("Warning: the word-labyrinth yields a faulty\n")
                    fil_chron.write("password. Alter it instead of reconstructing it.\n")
            print(f"A brief mention has been written in {what_log}.")
...
if (mode == "a") and not errorFlag:
    ...
    if (ans_CH == "y"):
        ...
        print(f"Book's location: {the_location}")
        "output: alteration"
        output_mode = input_choice("Camouflage trellis (t) or password shuttle (s)? ", \
                                   ["t", "s"])
        if (output_mode == "t"):
            camouflage_trellis_alteration(digits)
        elif (output_mode == "s"):
        ...
        if (antwoord_CH == "y"):
            ...
            print(f"\nA brief mention has been written in {what_log}.")
            if (output_mode == "t"):
                if (output_submode == "ss"):
                    print("If the password has just been assigned then record its metadata")
                    print("in the log for the password's domain, and make a backup or two")
                    print("of the file, including one in a flashdrive.\n")
                    print("Also play some mind-clearing game or puzzle.\n")
                elif (output_submode == "cc"):
                    print("Play some mind-clearing game or puzzle.\n")
                print("Close this window down when ending the session.\n")
...
if (mode == "a") and errorFlag:
    answer_CH = input_y_or_n("Would you like to enter \"swift alter\" mode (\"y\", \"n\")? ")
    if (answer_CH == "y"):
        print(f"Net result = {swift_alter(old_digits, -1)}")

        
if (mode == "r") and not errorFlag:
    if reconstruct_changed:
        ans_CH = input_y_or_n("\nSave the change(s) (y/n)? ")
        if (ans_CH == "y"):
    ...
    if (antwoord_CH == "y"):
        ...
        with open(what_log, "a") as fil_chron:
            ...
            if (output_mode == "t"):
                ...
                fil_chron.write("Output mode: wait to exchange\n")
        print(f"A brief mention has been written in {what_log}.")
        "reminders: reconstruction"
        if (output_mode == "t"):
            if (output_submode == "ss"):
                print("\nIf the password has just been assigned then record its")
                print("metadata in the log for the password's domain and make a")
                print("backup or two of the file, including one in a flashdrive.\n")
                print("Also play some mind-clearing game or puzzle.\n")
            elif (output_submode == "cc"):
                print("Play some mind-clearing game or puzzle.\n")
            print("Close this window down when ending the session.\n")          
    "swift alter mode?"
    is_for_exchange_NT = 0    # default
    if (output_mode == "w"):
        is_for_exchange_NT = 1
    elif (output_mode == "t"):
        if (output_submode == "cc"):
            is_for_exchange_NT = 0
        elif (output_submode == "ss"):
            is_for_exchange_NT = -1
    elif (output_mode == "s"):
        if (shuttle_pos == 1):
            is_for_exchange_NT = 0
            ...
            is_for_exchange_NT = -1
    if not ((output_mode == "t") and (output_submode == "ss") and (antwoord_CH == "y")):
        if not ((output_mode == "s") and (shuttle_pos == 2) and (antwoord_CH == "y")):
            answer_CH = ""
            if (output_mode == "w"):
                answer_CH = "y"
            if not (answer_CH in ["y", "n"]):
                answer_CH = input_y_or_n(\
