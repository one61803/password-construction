"This file is called OneHundredWords2SixteenAlphanumbers_3.py."
"It was forked from OneHundredWords2SixteenAlphanumbers_2.py and modified."
import random
import time

"A filename (of a file processed by the program) should have extension .1H16A."
"The output password has password entropy 16*lg(62) = 95.3."

"""SUMMARY OF PROGRAM'S ATTRIBUTES
Name: OneHundredWords2SixteenAlphanumbers_3.py
Extension of password constructing file: .1H16A
Input work level: 100 words
Output password entropy: 95.3 bits"""
...
def phi_ordnum(word_ST):
    ...
    if is_proper_keywordoid(word_ST):
        ...
        print("L156. ERROR in phi_ordnum: argument word_ST is not a proper wordoid.")
...
def if_page_then_swallow(a_string):
    ...
    if (a_string[0:3] == "P. ") and (a_string[3] in digits_LS):
        ...
        if (a_string[0] == " "):
            ...
            print("L264. ERROR in if_page_then_swallow: No space after the page number.")
    return a_string
...
def if_paragraph_then_swallow(a_string):
    ...
    if (a_string[0:5] == "PAR. ") and (a_string[5] in digits_LS):
        ...
        if (a_string[0] == " "):
            ...
            print("L275. ERROR in if_paragraph_then_swallow: No space after the paragraph number.")
    return a_string
...
if ans == "y":
    ...
    while not filename_OK:
        ...
        if (len(f_ST) < 7) or not (f_ST[-6:] == ".1H16A"):
            print("Error: The filename's extension should be .1H16A.")
        else:
            filename_OK = True
    with open(f_ST) as f:
        for line in f:
            ...
            if (line[0] == "(") and (line[-1] == ")"):
                ...
                if len(line_LS) == 4:
                    ...
                    if not line_LS[0] in ["Executor:", "Book:", "Author:", "Publisher:", "Location:", "Codeword hash:"]:
                        print("L337. ERROR: Not an approved first element for an ordered pair.")
                    if (line_LS[0] == "Executor:") and (not line_LS[1] == "OneHundredWords2SixteenAlphanumbers_3.py"): 
                    ...
                    print("L358. len(line_LS) = " + str(len(line_LS)))
                    print("L359. ERROR")
            else:
                print("L361. ERROR: Line is not wrapped in parentheses.")
    is_loaded = True
...
for i in range(1, upper_bound + 1):
    if not is_loaded:
        ...
        print("\nL443. ERROR: is_loaded should be Boolean.")
...
output = transcharacterization(vecsum_total_100)
...
if not is_loaded:
    ...
    if codeword_hash == the_codeword_hash:
        ...
        print("476. ERROR: Output's hash is NOT OK.")
...
if not is_loaded:
    ...
    while not ((ans_CH == "y") or (ans_CH == "n")):
        ans_CH = input("\nSave (y/n)? ")
    if ans_CH == "y":
        data.append(("Executor:", "OneHundredWords2SixteenAlphanumbers_3.py"))
        ...
        while not filename_OK:
            f_ST = input("Enter filename: ")
            if (len(f_ST) < 7) or not (f_ST[-6:] == ".1H16A"):
                print("Error: The filename should have extension .1H16A.")
            else:
                filename_OK = True
        ...
        print("METADATA")
        print("Filename: " + f_ST)
        print("Executor: OneHundredWords2SixteenAlphanumbers_3.py")        
