"This file is called FourHundredWords2FourDigits_10.py."
"It was forked from FourHundredWords2FourDigits_9.py and modified."
"""Objective of version 10: to create a new sub-mode of 'alter' mode
implemented through a new function called 'swift_alter', which would
use a new global list called 'thread', created during 'alter' mode.
Keywords inputted during 'alter' mode should now be stored in the
'thread' list. If there is a digit-crash at the end of alter mode,
then the user would enter swift-alter mode. [Note: 'thread' has
been replaced with 'keywords_LS', which coincides in name with a
list used in 'create new' mode: they have the same type of content
but it is used for different purposes.]"""
import random
import time
...
Name: FourHundredWords2FourDigits_10.py
...
def to_char(num):
    "Converts a number from 0 to 61 amphi-inclusive into a character of three types: (1) a digit, (2) an uppercase letter, (3) a lowercase letter. (The letters belonging to the English alphabet.)"
    if ((num >= 0) and (num <= 9)):
        return chr(48 + num)
...
def VAL(st):
    "Converts a string containing a number into the number contained by that string. The name 'VAL' is borrowed from BASIC."
...
def vector_hash_6(vector_LS):
    "Calculates a polynomial in x whose coefficients are components of vector_LS, using Horner's method, for the purpose of providing a hash for a string which is encoded in vector_LS."
    x = 31
    Z = 999983
    for i in range(16):
        if (i == 0):
...
def swift_alter():
    ...
    if not errorFlag:
        ...
        if (ans_CH == "n"):
            ...
            print("Executor: FourHundredWords2FourDigits_10.py")
...
for i in range(1, upper_bound + 1):
    if (mode == "c"):
        ...
        print("L1226. ERROR: variable mode should be one of c, r, a.")
...
if (mode == "r"):
    if (codeword_hash == the_codeword_hash):
        ...
        print("L1309. ERROR: Output's hash is NOT OK.")
...
if (mode == "c"):
    ...
    if (ans_CH == "y"):
        data.append(("Executor:", "FourHundredWords2FourDigits_10.py"))
