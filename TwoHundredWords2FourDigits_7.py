"This file is called TwoHundredWords2FourDigits_7.py."
"It was forked from SixHundredWords2FourDigits_3.py."
...
from array import *
my_filename = "TwoHundredWords2FourDigits_7.py"
my_associated_file_extension = ".2H4D"
...
Name: TwoHundredWords2FourDigits_7.py
...
def camouflage_trellis_modulator(digits_AR):
...
request, then deal with it; otherwise give an error message."
    ...
    if not is_proper_keyword(word_ST):
        if (i > 1) and (word_ST == ":CORRECTION:"):
            ...
            out_ST += f"{data[i - 1][0]}"
            print(out_ST)
            prependix = input("Enter the word(s) to prepend: ")
            while not is_proper_addition(prependix):
                print("Error: The addition should be a string made up of uppercase")
...
if (not errorFlag) and (mode == "r") and (output_mode == "t"):
    camouflage_trellis_modulator(digits)
...
if (mode == "r") and not errorFlag:
    ...
    if not ((output_mode == "t") and (output_submode == "ss") and (antwoord_CH == "y")):
        if not ((output_mode == "s") and (shuttle_pos == 2) and (antwoord_CH == "y")):
            answer_CH = ""
            while not (answer_CH in ["y", "n"]):
                out_ST = "Would you like to enter \"swift alter\" mode (\"y\", \"n\")? "
                answer_CH = input(out_ST)
                if not (answer_CH in ["y", "n"]):
                    print("Error: please enter either \"y\" or \"n\".")
            if (answer_CH == "y"):
